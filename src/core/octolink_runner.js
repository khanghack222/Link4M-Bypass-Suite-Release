...
async function runBypass(octolinkUrl, options = {}, onLog = console.log) {
  ...
  try {
        ...
        const hit = await page.evaluate((sNum) => {
          ...
          for (const c of qContainers) {
            ...
            if (isVisible(target)) {
              ...
              if (isCountingOrWaiting(text)) continue;
              const norm = text.toUpperCase().replace(/\s+/g, '');
              if (!norm.includes('LẤY') && !norm.includes('LAY') && !norm.includes('STEP') && !norm.includes('BƯỚC') && !norm.includes('BUOC') && !norm.includes('CODE') && !norm.includes('MÃ')) continue;
    ...
    for (let currentStep = 1; currentStep <= MAX_STEPS; currentStep++) {
      ...
      for (let i = 0; i < 90; i++) {
        ...
        try {
          ...
          // 4. Dò tìm giây đếm ngược trong regular DOM nếu Shadow DOM chưa có
          if (second === null) {
      ...
      if (finalUrl.includes('link999.app/finish') || finalUrl.includes('link999.app') || finalUrl.includes('uptolink') || finalUrl.includes('octolink.vip/finish') || finalUrl.includes('octolink.vip')) {
        ...
        if (hasCaptchaElement) {
          ...
          if (captchaSolved) {
            ...
            onLog(`🚀 Captcha đã giải xong! Đang chờ máy chủ xác thực và chuyển sang view nhận link đích...`);
            await activePage.evaluate(() => {
            ...
            // Chờ trang tự chuyển sang view Get-Link (qua script gốc của hold_captcha)
            let getLinkReady = false;
            const waitGetLinkStart = Date.now();
            while (Date.now() - waitGetLinkStart < 25000) {
              ...
              // Sau 5s nếu chưa thấy chuyển và token vẫn còn, thử kích hoạt click tự nhiên 1 lần
              if (Date.now() - waitGetLinkStart > 5000) {
                await activePage.evaluate(() => {
                  if (!window.__singleClickTriggered) {
                    window.__singleClickTriggered = true;
        ...
        // Kích hoạt nhận link đích trực tiếp qua AJAX /links/go2 với FormData chuẩn
        onLog(`⚡ Đang gửi request giải mã nhận Real Callback URL từ /links/go2...`);
        const directGoRes = await activePage.evaluate(async () => {
          window.blurred = false;
          const goForm = document.getElementById('go-link');
          if (!goForm) return null;
          goForm.classList.add('go-link');
          const action = goForm.getAttribute('action') || '/links/go2';
        ...
        if (directGoRes && directGoRes.url && isValidFinalDestination(directGoRes.url, targetDomain)) {
          finalDestinationUrl = directGoRes.url;
          finalUrl = directGoRes.url;
          onLog(`🌟 [THÀNH CÔNG RỰC RỠ] Đã nhận Real Callback URL từ /links/go2: ${finalUrl}`);
        } else {
          // Dự phòng nếu fetch chưa có kết quả, kích hoạt jQuery submit gốc
          await activePage.evaluate(() => {
            const goForm = document.getElementById('go-link');
            if (goForm && window.$) {
    ...
    if (isSuccess) {
      try {
        const destPaths = [
          path.join(__dirname, '..', 'destination_url.txt'),
          path.join('C:\\Users\\XUAN\\.gemini\\antigravity\\scratch\\Link4M_Bypass_Suite', 'destination_url.txt'),
          path.join('C:\\Users\\XUAN\\.gemini\\antigravity\\scratch\\Link4M-Bypass-Suite-Release', 'destination_url.txt')
        ];
        for (const dp of destPaths) {
          if (fs.existsSync(path.dirname(dp))) {
            fs.writeFileSync(dp, finalUrl, 'utf-8');
          }
        }
      } catch (e) {}
    }
