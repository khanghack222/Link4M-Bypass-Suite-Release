(function() {
	var traffic_key = "\x58\x57\x47\x58\x6D\x42\x62\x42";
	var traffic_id = "\x35\x38\x38\x35\x30\x32\x66\x66\x35\x30\x35\x36\x39\x61\x30\x30\x37\x32\x37\x39\x61\x34\x35\x32\x34\x34\x31\x61\x31\x36\x36\x30";
	var traffic_domain = 's1.what-on.com';
	var v3ab04 = '6aa4f0030f5da4f0a00fd83a';
	var uuid_name = '\x74\x72\x61\x66\x66\x69\x63\x5F\x70\x67\x45\x5A\x74\x58\x57\x47\x58\x6D\x42\x62\x42';
				...
				button.addEventListener(clickHandler, function(){
					...
					detectIncognito().then((result) => {
						if(result.isPrivate){
							...
							var params = 'traffic_session=' + v3ab04 + '&key=' + traffic_key + '&' + jscd_param;
							xmlhttp.send(params);
		...
		if((element.dataset.loading == undefined || element.dataset.loading == 'false')
			...
			switch(step){
				case 2 :
					...
					var jscd_param =  new URLSearchParams(jscd).toString();
					xmlhttp.open("GET", "https://" + traffic_domain + "/widget/get_quest_code.html?id=" + quest_info.id + "&code=" + traffic_id + '&traffic_session=' + v3ab04 + '&key=' + traffic_key + '&' + jscd_param, true);
					xmlhttp.send();
					break;
				case 1:
				default:
					...
					var jscd_param =  new URLSearchParams(jscd).toString();
					xmlhttp.open("GET", "https://" + traffic_domain + "/widget/get_quest_code.html?code=" + traffic_id + '&traffic_session=' + v3ab04 + '&key=' + traffic_key + '&' + jscd_param, true);
					xmlhttp.send();
					break;
